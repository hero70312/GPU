...
int main()
{
	char **tt = (char **)malloc(sizeof(char *)*7);
	tt[0]= "F:\\Hand_Project\\GPU\\GPGPU_Programming_2016S\\bin\\win64\\Debug\\template.exe";
	tt[1] = "F:\\Hand_Project\\GPU\\GPGPU_Programming_2016S\\bin\\win64\\Debug\\img_background.ppm";
	tt[2] = "F:\\Hand_Project\\GPU\\GPGPU_Programming_2016S\\bin\\win64\\Debug\\img_target.ppm";
	tt[3] = "F:\\Hand_Project\\GPU\\GPGPU_Programming_2016S\\bin\\win64\\Debug\\img_mask.pgm";
	tt[4] = "130";
	tt[5] = "600";
	tt[6] = "F:\\Hand_Project\\GPU\\GPGPU_Programming_2016S\\bin\\win64\\Debug\\output.ppm";
	/*
	if (argc != 7) {
	...
	auto imgb = ReadNetpbm(wb, hb, cb, sucb, tt[1]); //backgound
	auto imgt = ReadNetpbm(wt, ht, ct, suct, tt[2]); //target
	auto imgm = ReadNetpbm(wm, hm, cm, sucm, tt[3]); //mask
	if (!(sucb && suct && sucm)) {
	...
	const int oy = atoi(tt[4]), ox = atoi(tt[5]);
